Public Class Domicilio
	Shared Function Listar(persona_id As Integer) As DataTable
		Return DbMan.read(Nothing, My.Settings.DefaultCon, "SELECT per_domicilio.id as domicilio_id, per_domicilio.calle, per_domicilio.altura,
	...
	Shared Function Nuevo(persona_id As Integer, calle As String, altura As Integer, piso As Integer,
		...
		DbMan.edit(Nothing, My.Settings.DefaultCon, "INSERT INTO per_domicilio(per_id, calle, altura, piso, dpto, localidad_id, principal)
										   VALUES(" & persona_id & ", '" & calle & "', " & altura & ", " & piso & ",
												  '" & dpto & "', " & localidad_id & ", " & principal & ")")

		Dim dtab As DataTable = DbMan.read(Nothing, My.Settings.DefaultCon, "SELECT MAX(id) as id FROM per_domicilio WHERE per_id=" & persona_id)
		Return dtab(0)("id")
	End Function
	Shared Function Modificar(domicilio_id As Integer, persona_id As Integer, calle As String,
		...
		DbMan.edit(Nothing, My.Settings.DefaultCon, "UPDATE per_domicilio SET per_id=" & persona_id & ", calle='" & calle & "', altura=" & altura & ",
												 piso=" & piso & ", dpto='" & dpto & "', 
