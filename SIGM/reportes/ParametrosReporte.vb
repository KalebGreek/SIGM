    ...
    Shared Function TableToReport(dtab As DataTable, titulo As String, parametros As List(Of ReportParameter)) As List(Of ReportParameter)
        With parametros
            For Each col As DataColumn In dtab.Columns
                'Add original name to header param
                .Add(New ReportParameter("ColumnTitle" & col.Ordinal + 1, col.ColumnName))
                'Rename colmun to make it compatible with the report
                col.ColumnName = "Column" & col.Ordinal + 1
            Next
            .Add(New ReportParameter("ReportTitle", titulo))
        End With
